...
def create_base_docx(content, header, filename):
    ...
    document.add_paragraph(content)  # This is where we will add the content generated by the GenAi Tool.
...
def inject_canary_xml(filename, tracking_url, rel_id):
    ...
    try:
        ...
        if body is not None:
            # Create a new paragraph containing a VML image reference
            ...
            shape_tag = etree.SubElement(
                pict_tag,
                etree.QName(NAMESPACES['v'], 'shape'),
                id="_x0000_i1025",
                type="#_x0000_t75",
            ...
            etree.SubElement(
                shape_tag,
                etree.QName(NAMESPACES['v'], 'imagedata'),
                {
        ...
        new_zip_filename = filename.replace(".docx", "_canary.docx")
        ...
        # Replace original file with canary version
        if os.path.exists(filename):
            os.remove(filename)
        os.rename(new_zip_filename, filename)
        print(f"Document injected and saved as '{filename}'.")
